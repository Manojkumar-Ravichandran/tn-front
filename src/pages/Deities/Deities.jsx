...
/* ─── Deity pill card ─── */
const DeityPill = ({ deity, isActive, onClick }) => (
    <button
        onClick={onClick}
...
const TempleCard = ({ temple, index }) => {
    ...
    return (
        <Link
            ...
            className="group relative bg-background border border-border-theme rounded-[1.75rem] overflow-hidden hover:shadow-xl hover:border-purple-500/20 transition-all duration-300 active:scale-[0.98]"
        ...
        >
            ...
            {/* Info */}
            <div className="p-5">
                <h3 className="font-black text-foreground text-base leading-tight mb-2 group-hover:text-purple-500 transition-colors line-clamp-1">
                    {temple.name}
                </h3>
...
const Deities = () => {
    ...
    const { data: deities = [], isLoading: loadingDeities } = useQuery({
        queryKey: ['deities'],
        queryFn: getDeities,
    });

    const effectiveDeity = selectedDeity || (deities.length > 0 ? deities[0] : null);

    const { data: temples = [], isLoading: loadingTemples } = useQuery({
        queryKey: ['temples-by-deity', effectiveDeity?._id],
        queryFn: () => getTemplesByDeity(effectiveDeity?._id),
        enabled: !!effectiveDeity,
    });
            ...
            <section className="pt-24 pb-8">
                <div className="container mx-auto px-6 max-w-7xl">
                    <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-foreground/30 mb-6">
                        ...
                        <span>/</span>
                        <span className="text-foreground/70">Deities</span>
                    </nav>
                    ...
                    <div className="flex flex-col md:flex-row md:items-end gap-4 justify-between mb-8">
                        <div>
                            <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tight leading-tight mb-2">
                                Explore by <span className="text-purple-500 italic">Deity</span>
                            </h1>
                            <p className="text-foreground/40 font-medium">
                                Select a deity to discover temples dedicated to that divine power.
                            </p>
                        </div>

                        {/* Search */}
                        <div className="relative w-full md:w-72 group">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-foreground/30 group-focus-within:text-purple-500 transition-colors" />
                            <input
                                value={search}
                                onChange={e => setSearch(e.target.value)}
                                placeholder="Search temples..."
                                className="w-full bg-secondary-bg border border-border-theme focus:border-purple-400/40 pl-11 pr-4 py-3 rounded-2xl text-sm focus:outline-none transition-all placeholder:text-foreground/25 font-medium"
                            />
                        </div>
                    </div>

                    {/* ── DEITY HORIZONTAL SCROLL ── */}
                    <div className="relative mb-10">
                        ...
                        <div
                            ref={scrollRef}
                            className="flex items-center gap-3 overflow-x-auto scroll-smooth px-4 py-2"
                            ...
                            {loadingDeities
                                ...
                                : deities.map(d => (
                                    <DeityPill
                                        key={d._id}
                                        deity={d}
                                        isActive={effectiveDeity?._id === d._id}
                                        onClick={() => { setSelectedDeity(d); setSearch(''); }}
                                    />
                                ))
                            }
                        </div>
                    ...
                    {effectiveDeity && (
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h2 className="text-lg font-black text-foreground">
                                Temples of{' '}
                                <span className="text-purple-500">{effectiveDeity.name}</span>
                            </h2>
                            {!loadingTemples && (
                        ...
                        <div className="py-24 text-center border-2 border-dashed border-border-theme rounded-[2.5rem] bg-secondary-bg/10">
                            ...
                            <p className="text-lg font-black text-foreground/30">
                                {search
                                    ...
                                    : `No approved temples for ${effectiveDeity?.name} yet`}
                            </p>
                            {search && (
                                <button
                                    onClick={() => setSearch('')}
                                    className="mt-4 text-xs font-black uppercase tracking-widest text-purple-500 hover:underline"
                                >
                                    Clear search
                                </button>
                            )}
